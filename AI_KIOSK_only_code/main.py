...
def base_menu_name(menu_name):
    menu_name = str(menu_name or "").strip()
    return menu_name[4:].strip() if menu_name.startswith("ICE ") else menu_name


def extract_order_menus_from_text(text):
    text = str(text or "").strip()
    if not text:
        return []

    menu_names = [str(name).strip() for name in df["상품명"].tolist() if str(name).strip()]
    menu_by_name = {name: name for name in menu_names}
    base_names = sorted({base_menu_name(name) for name in menu_names}, key=len, reverse=True)
    compact = "".join(text.split())
    matches = []
    occupied = []

    for base in base_names:
        compact_base = "".join(base.split())
        if not compact_base:
            continue

        start = 0
        while True:
            index = compact.find(compact_base, start)
            if index == -1:
                break
...
def split_order_segments(text):
    text = str(text or "").strip()
    if not text:
        return []

    temp_tokens = r"ICE|HOT|아이스|아이쓰|아이수|차가운|시원한|따뜻한|뜨거운|핫"
    segmented = re.sub(rf"\s+(?=(?:{temp_tokens})\s*)", "\n", text, flags=re.IGNORECASE)
    segmented = re.sub(r"(?:,|/|\+|랑|하고|이랑|와|과|그리고|및|에다가)", "\n", segmented)
    return [segment.strip() for segment in segmented.splitlines() if len(segment.strip()) >= 2]


def extract_fuzzy_order_menus_from_text(text):
    ordered = []
    seen = set()
    ...
    for segment in split_order_segments(text):
        segment_menus = extract_order_menus_from_text(segment)
        if segment_menus:
            for menu_name in segment_menus:
                add_menu(menu_name)
            continue

        result = search_menu(segment)
        if len(result) == 1:
            add_menu(result[0])

    if not ordered:
        for menu_name in extract_order_menus_from_text(text):
            add_menu(menu_name)

    return ordered


def extract_multiple_order_menus(*texts):
    for text in texts:
        menus = extract_order_menus_from_text(text)
        if len(menus) > 1:
            return menus

    for text in texts:
        menus = extract_fuzzy_order_menus_from_text(text)
        if len(menus) > 1:
            return menus
    return []


def generate_multi_order_response(menus):
    menu_text = ", ".join(display_menu_name(menu) for menu in menus)
    option_lines = "\n".join(
        f"{display_menu_name(menu)}: {get_default_option_summary(menu)}"
        for menu in menus
    )
...
def correct_confirmation_answer(text):
    ...
    if compact in positive_misheard:
        return "네, 맞아요"
    if compact.startswith(("네마자", "내마자", "넵마자", "예마자", "네맞", "내맞", "넵맞", "예맞")):
        return "네, 맞아요"
    if compact.startswith(("아녀", "안니", "아뇨", "아니")):
        return "아니요"
    return text
