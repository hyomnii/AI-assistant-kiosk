...
def generate_kiosk_response(menu_results, user_query):
    ...
    if len(menu_results) >= 5:
        # 메뉴가 많으면 화면에는 전체 목록을 보여주고 다음 줄에서 선택을 요청한다.
...
def get_default_option_summary(menu_name):
    ...
    return ", ".join(options) + "으로 선택되었습니다."
...
def speak_tts(text, tts_text=None):
    ...
    tts = gTTS(normalize_tts_text(tts_source), lang="ko")
    ...
    while True:
        try:
            raw_text = listen_with_beamforming(
                ...
                allow_short_confirmation=conversation_state in {"CONFIRM_MENU", "OPTION_SELECT"},
            )
        ...
        if conversation_state == "CONFIRM_MENU":
            ...
            if confirmation == "yes":
                ...
                # 디저트는 음료 옵션 선택 단계 없이 주문 흐름을 종료한다.
                if not is_drink_menu(pending_menu):
                    ...
                    speak_tts(
                        "어떤 옵션을 선택하시겠어요?\n"
                        "옵션 선택을 원하시지 않으시면 모두 기본으로 라고 말씀해주세요.\n"
                        "원하시는 옵션이 있으시면 해당 옵션을 선택해주세요."
                    )
                    option_text = listen_for_default_options_with_countdown(stt_model)
                    if option_text:
                        print_order(option_text)
                    # 기본 옵션 발화가 잡히면 추가 선택 없이 기본값으로 확정한다.
                    if is_default_options_answer(option_text):
                        speak_tts(get_default_option_summary(pending_menu))
                        conversation_state, pending_menu, pending_candidates = reset_order_state()
                        wait_for_enter = True
                    else:
