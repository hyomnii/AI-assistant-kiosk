...
<body>
      ...
      <aside class="ai-log">
        ...
        <div class="ai-order-box" id="aiOrderBox" hidden>
          <div class="ai-order-head">
            <strong id="aiOrderName"></strong>
            <div class="cart-qty" aria-label="AI order quantity controls">
              <button class="qty-btn" id="aiQtyMinus" type="button" aria-label="Decrease quantity">-</button>
              <span class="qty-count" id="aiQtyCount">1</span>
              <button class="qty-btn" id="aiQtyPlus" type="button" aria-label="Increase quantity">+</button>
            </div>
          </div>
          <div class="ai-order-options" id="aiOrderOptions"></div>
          <div class="ai-actions">
            <button class="secondary" id="changeOptions" type="button">옵션 변경</button>
  ...
  <script>
    ...
    const aiOrderName = document.querySelector("#aiOrderName");
    const aiOrderOptions = document.querySelector("#aiOrderOptions");
    const aiQtyMinus = document.querySelector("#aiQtyMinus");
    const aiQtyCount = document.querySelector("#aiQtyCount");
    const aiQtyPlus = document.querySelector("#aiQtyPlus");
    const changeOptions = document.querySelector("#changeOptions");
    ...
    document.querySelector("#addCart").addEventListener("click", () => {
      if (screens.ai.classList.contains("active") && selectedMenu) {
        ...
        aiOrderName.textContent = selectedMenu.name;
        aiOrderOptions.textContent = Object.entries(selectedOptions)
    ...
    changeOptions.addEventListener("click", async () => {
      ...
      setListening(false);
      currentAudio?.pause();
      await openAiOptionEditor();
    });

    aiQtyMinus.addEventListener("click", () => {
      if (currentAiQuantity <= 1) return;
      currentAiQuantity -= 1;
      renderAiQuantity();
    });

    aiQtyPlus.addEventListener("click", () => {
      currentAiQuantity += 1;
      renderAiQuantity();
    });

    async function openAiOptionEditor() {
      if (!currentAiMenu) {
        addMessage("assistant", "옵션을 변경할 메뉴를 찾지 못했어요.");
        return;
      }
      await loadMenu();
      const item = menuItems.find((menu) => menu.name === currentAiMenu);
      if (!item) {
        addMessage("assistant", "해당 메뉴의 옵션 정보를 찾지 못했어요.");
        return;
      }
      openOptions(item);
    }
    ...
    payAi.addEventListener("click", async () => {
      stopVoiceOrderFlow();
      aiOrderBox.hidden = true;
      addMessage("assistant", "주문이 완료되었습니다. 새로운 주문을 원하시면 음성 안내 시작 버튼을 눌러주세요.");
      await resetConversation();
      voiceTitle.textContent = "새 주문을 시작할 수 있어요.";
      voiceHelp.textContent = "새로운 주문을 원하시면 음성 안내 시작 버튼을 눌러주세요.";
      currentAiMenu = null;
      currentAiQuantity = 1;
    });

    function showAiOrderBox(data) {
      const menuName = data.pending_menu || currentAiMenu || "";
      currentAiMenu = menuName || currentAiMenu;
      aiOrderName.textContent = menuName;
      aiOrderOptions.textContent = formatAiOptionText(data.default_options || "");
      renderAiQuantity();
      aiOrderBox.hidden = false;
    }

    function renderAiQuantity() {
      aiQtyCount.textContent = currentAiQuantity;
      aiQtyMinus.disabled = currentAiQuantity <= 1;
    }

    function formatAiOptionText(defaultOptions) {
      if (!defaultOptions) return "기본 옵션";
