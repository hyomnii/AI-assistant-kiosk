...
# 4. LLM 1차 보정
# -------------------------------
def llm_correct(query, context=None):
...
문맥 보정 규칙:
- 사용자가 메뉴를 고르기 전이면, 카페 키오스크에서 할 법한 메뉴 탐색/추천/목록 질문으로 해석하세요.
- 음성인식 결과가 칭찬이나 일상 대화처럼 보여도, 발음상 가까운 메뉴 탐색 질문이 더 자연스러우면 그 질문으로 교정하세요.
- 단, 문맥상 확실하지 않으면 억지로 메뉴명이나 질문으로 바꾸지 마세요.
"""
...
# -------------------------------
def correct_text(query, context=None):
    ...
    if direct_corrected != query:
        ...
        query = llm_correct(query, context=context)
