...
def has_positive_constraint(query):
    return any(token in query for token in POSITIVE_CONSTRAINTS)
...
def has_allergen_context(query):
    return "알레르기" in query or "알러지" in query or "알레르겐" in query


def extract_allergens(query):
    if not has_filter_context(query):
        return []

    mentioned = []
    for allergen in ALLERGENS:
        aliases = ALLERGEN_ALIASES.get(allergen, [allergen])
        if any(alias in query for alias in aliases):
            mentioned.append(allergen)
    return mentioned
...
def extract_conditions(raw_query, corrected_query=None):
    ...
    mentioned_allergens = extract_allergens(raw_query)
    exclude_allergen = has_negative_constraint(raw_query)
    allergen_mode = None
    if has_allergen_context(raw_query) and not mentioned_allergens:
        if exclude_allergen:
            allergen_mode = "NONE"
        elif has_positive_constraint(raw_query) or has_filter_context(raw_query):
            allergen_mode = "ANY"
...
def extract_temp(query):
    ...
    if "hot" in q or any(x in query for x in ["따뜻한", "뜨거운", "핫"]) or has_standalone_token(query, "뜨아"):
        return "HOT"
...
def apply_condition_filters(candidates, conditions):
    ...
    if conditions["category"]:
        if conditions["category"] == "음료":
            result = [c for c in result if c["category"] in {"커피", "차"}]
        else:
    ...
    if conditions["allergen_mode"] == "NONE":
        result = [c for c in result if not c["allergy"]]
    elif conditions["allergen_mode"] == "ANY":
        result = [c for c in result if c["allergy"]]
