...
client = PersistentClient(
    path=r"C:\Users\user3\Desktop\학연생\종합설계\AI-assistant-kiosk\menu_DB"
)
...
def extract_category(query):
    if "커피" in query:
        return "커피"
    if "빵" in query or "디저트" in query or "케익" in query:
        return "디저트"
    if "차" in query:
        return "차"
    if "에이드" in query:
