...
# 오인식 보정 + 모델 공유
from stt_correction_model import correct_text, model
...
def extract_conditions(query):
    ...
    for k in base_keywords:
        match, score, _ = process.extractOne(query, [k], scorer=fuzz.partial_ratio)
        if score > 70 and k not in keywords:
            keywords.append(k)
